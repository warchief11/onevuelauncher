...
namespace OneVueLauncher
{
    public partial class frmLauncher
    {
        ...
        private OrderedDictionary _branches = new OrderedDictionary();
        private string _working_dir_path = "c:\\svn_testing";
         internal static string GitWorkingDirPath = "c:\\git_testing";
        private string _repository_uri = "svn://launcher.fas.au.challenger.net/fas/onevuereleases2009/";
        internal static string Repository_uri_git = @"https://github.com/warchief11/";
        ...
        private Branch SelectedBranch
        {
            get
            {
                if (_branches != null && _branches.Count > _currentBranchIndex)
                {
                    return _branches[_currentBranchIndex] as Branch;
                }
                return null;
            }
        }
                    ...
                    foreach (XmlNode baseData_loopVariable in baseDataNodes)
                    {
                        ...
                        switch (baseData.Name)
                        {
                            ...
                            case "BranchWorkingDirectory":
                                ...
                                break;
                            case "SourceControl":
                                newBranch.SourceControl = baseData.InnerText.ToUpper() == "GIT" ? SourceControl.Git : SourceControl.Svn;
                                break;
                        }
                    }
        ...
        private void LoadComboWithBranch(ComboBox cbo, OrderedDictionary branches)
        {
            cbo.Items.Clear();
            if (branches.Count > 0)
            {
                ...
                {
                    cbo.Items.Add(branch.BranchDisplayName);
                }
            }
        }
        ...
        private void frmLauncher_Load(object sender, System.EventArgs e)
        {
            ...
            LoadComboWithBranch(cboBranches, _branches);
        ...
        private void UpdateBranch()
        {
            ...
            try
            {
                ...
                frmCheckout.CheckoutBranch = SelectedBranch;
        ...
        private void radTraining_CheckedChanged(System.Object sender, System.EventArgs e)
        {
            if (radTraining.Checked)
            {
                ...
                LoadComboWithBranch(cboTraining, _branches);
                this.Cursor = Cursors.Default;
                update_revision_info();
            }
        }
        ...
        private void update_revision_info()
		{
			...
			try {
				...
				if (!string.IsNullOrEmpty(lblDisplayName.Text) | radProduction.Checked) {
                    ...
                    if (SelectedBranch != null && SelectedBranch.SourceControl == SourceControl.Git)
                    {
                       var workingDir = GitWorkingDirPath + SelectedBranch.BranchName;
                       var sourceURL = frmLauncher.Repository_uri_git + SelectedBranch.BranchName;
                       var remoteReferences = Repository.ListRemoteReferences(sourceURL);
                    ...
                    else
                    {
                        ...
                        {
                            this.lblRevisionInfo.Text = last_changed_datetime.ToString(); 
                        }
        ...
        private void radGitTesting_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;
                if (radGitTesting.Checked)
                {
                    radProduction.Checked = false;
                    //radBranches.Checked = false;
                    cboTraining.Enabled = true;

                    if (string.IsNullOrEmpty(System.Configuration.ConfigurationManager.AppSettings.Get("Git_TestingPath")))
                    {
                        LoadData(_currentDirectory + "\\Git_Testing.xml");
                    }
                    else
                    {
                        LoadData(System.Configuration.ConfigurationManager.AppSettings.Get("Git_TestingPath"));
                    }

                    LoadComboWithBranch(cboGitTesting, _branches);
                    update_revision_info();
                }
                else
                {
                    cboGitTesting.Enabled = false;
                    cboGitTesting.SelectedIndex = -1;
                }
            }
            finally
            {
